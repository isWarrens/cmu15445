...
auto HashTableDirectoryPage::GetGlobalDepthMask() -> uint32_t { return (2 << global_depth_) - 1; }
...
auto HashTableDirectoryPage::Size() -> uint32_t { return directory_size_; }

auto HashTableDirectoryPage::CanShrink() -> bool {
  auto max_index = GetGlobalDepthMask();
...
auto HashTableDirectoryPage::GetLocalHighBit(uint32_t bucket_idx) -> uint32_t {
  return bucket_idx >> (global_depth_ - local_depths_[bucket_idx]);
}
